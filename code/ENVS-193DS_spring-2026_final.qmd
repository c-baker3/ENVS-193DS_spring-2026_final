...
- Tree height is a continuous predictor variable (centimeters). As tree height increases, the probability of nest presence in the tree will likely increase because taller trees provide more protection from predators on the ground; taller trees also offer more canpoy structure for effective nest placement This reasoning was found WHERE. 
-- FINISH WITH CITATION. 

- Acacia ant species is a categorical predictor variable with three levels: C. sjostedti, C. mimosae, and C. nigriceps. Ant species type could predict nest occurrence because different species display different levels of aggressiveness and have different colony sizes; these factors may either attract or deter nesting birds depending on whether the ants provide protection from predators or themselves threaten the birds. This reasoning was found WHERE. 
-- FINISH WITH CITATION
...
**Conclusion**: The best model as determined by Akaike's Information Criterion (AIC) is model 2 which displays the presence of nests as a function of tree height multiplied by ant species. CHECK FOR CORRECTNESS. 
...
Tree height and ant species interact to predict probability of bird nest occurrence. As tree height increases, the probability of nest occurrence increases across presence of all three ant species, suggesting that taller trees are generally better nesting sites for birds regardless of ant species. 

Additionally, the effect of tree height on nest occurrence probability differs across ant species, with C. nigriceps showing the strongest relationship (odds ratio: [X]) and C. sjostedti showing the weakest (odds ratio: [X]).
-- FILL IN EFFECT SIZE FROM ODDS RATIO??

At 600 cm, the predicted probability of nest occurrence is 0.20 (95% CI [0.00, 0.99]) for C. sjostedti, 0.74 (95% CI [0.48, 0.90]) for C. mimosae, and 1 (95% CI [1.00, 1.00]) for C. nigriceps.
