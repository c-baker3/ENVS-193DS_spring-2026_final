...
ggplot(data = nests_clean, # data frame: nests_clean
  ...
  geom_ribbon(data = preds, # data frame: preds
              mapping = aes(
                        ...
                        ymin = conf.low, 
                        ymax = conf.high,
                        fill = species_name), # colored ribbon by ant species
               alpha = 0.1) + # made ribbon more transparent
  # created panel for each ant species
  facet_wrap(~ species_name,
             # labeled each panel by species name
...
**Figure 1. Probability of bird nest occurrence increases with tree height across all three acacia ant species.** Open circles represent individual tree observations (1 = nest present, 0 = nest absent), colored by ant species. Solid lines represent model predicted probability of nest occurence across the range of observed tree heights, and shaded ribbons represent 95% confidence intervals. 
...
# what is the probability of nest occurrence at a value of 600cm (the tallest tree in the dataset?
ggpredict(model2,
...
At the tallest tree in the dataset (600 cm), the predicted probability of nest occurrence was 0.20 for C. sjostedti (95% CI [0.00, 0.99]), 0.74 for C. mimosae (95% CI [0.48, 0.90]), and 1 for C. nigriceps (95% CI [1.00, 1.00]) (Figure 1). Across all three ant species, probability of nest occurrence increased with tree height, suggesting that taller trees are generally more attractive nesting sites for birds regardless of ant identity. However, ant species modulated this relationship, with [highest species] showing the steepest increase in nest probability with tree height and [lowest species] showing the shallowest. This pattern is consistent with the idea that ant species vary in their aggressiveness and ability to deter nest predators — birds may preferentially nest in trees occupied by more protective ant species, particularly as tree height increases and nest sites become more exposed.
