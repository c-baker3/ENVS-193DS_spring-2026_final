...
- Tree height is a continuous predictor variable measured in centimeters. As tree height increases, the probability of nest occurrence in the tree would increase because taller trees provide more protection from predators on the ground that threaten the nesting birds (Results, paragraph 2). Taller trees may also offer more canopy structure for nest placement (Discussion, paragraph 5). 

- Acacia ant species is a categorical predictor variable with three levels: C. sjostedti, C. mimosae, and C. nigriceps. Ant species type could predict nest occurrence because different ant species vary in their aggressiveness towards on the ground mammals that threaten the nesting birds, changing the amount of protection the nesting birds receive (Introduction, paragraph 2).  While C. mimosae and C. nigriceps aggressively defend trees against mammals, insect herbivores, and lethal herbivory by elephants, C. sjostedti only provides minimal protection against herbivory (Introduction, paragraph 2). Therefore, it is logical that nesting birds would prefer trees with C. mimosae and C. nigriceps colonies present showing mutualism (rephrase rooted more in the paper itself). 
...
# model 2: interaction term (tree height x ant species)
...
**Conclusion**: The best model as determined by Akaike's Information Criterion (AIC) is model 2 which displays the presence of nests as a function of tree height multiplied by ant species. 
...
# generated model predictions across range of height values for each ant
# species
preds <- ggpredict(
...
ggplot(data = nests_clean, # data frame: nests_clean
  ...
  geom_line(data = preds,                # data frame: preds for model 2
            mapping = aes(x = height_cm, # x-axis: height (cm)
                          ...
                          color = species_name), # colored line by ant species
            linewidth = 2) +             # made line wider
...
Tree height and ant species interact to predict probability of bird nest occurrence. As tree height increases, the probability of nest occurrence increases across presence of all three ant species, suggesting that taller trees are generally better nesting sites for birds regardless of ant species. At 600 cm, the predicted probability of nest occurrence is 0.20 (95% CI [0.00, 0.99]) for C. sjostedti, 0.74 (95% CI [0.48, 0.90]) for C. mimosae, and 1 (95% CI [1.00, 1.00]) for C. nigriceps. 

-- ADD ONE OR TWO MORE SENTENCES BASED ON PROMPT TO HIT ALL MARKS -- 
...
The assumptions for a binomial GLM are met. First, each workout entry represents a separate day, so observations are independent. Second, there are binary outcomes, seen through the response variable run having 1 (a run occurred) and 0 (a run did not occur) outcomes. Finally, the diagnostic plots (simulated below) do not flag any issues with distribution of residuals or presence of influential outliers. 
...
# what is the probability of going on a run after sleeping for 7 hours (the average sleep duration in my dataset)?
ggpredict(run_mod,
          terms = "sleep_hours [7]")
```
